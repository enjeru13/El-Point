...
import { verificationPhotoUrl } from "@/lib/storage";
import type {
  RestaurantReportReason,
...
export function useRestaurantQueue(enabled: boolean) {
  return useQuery({
    queryKey: QUEUE_KEY,
    queryFn: fetchPendingRestaurants,
    enabled,
  });
}
...
export function useReportedRestaurants(enabled: boolean) {
  return useQuery({
    queryKey: REPORTS_KEY,
    queryFn: fetchReportedRestaurants,
    enabled,
  });
}
    ...
    mutationFn: async ({
      ...
      const { error } = await supabase
        ...
        .update({
          ...
          status_reason:
            action === "restore" ? null : "Retirado tras revisión de reportes",
          reviewed_at: new Date().toISOString(),
        })
        .eq("id", restaurantId);
      if (error) throw error;
...
export function useAdminCounts(enabled: boolean) {
  return useQuery({
    ...
    queryFn: async (): Promise<AdminCounts> => {
      const [reviews, restaurants, reported] = await Promise.all([
        supabase
          .from("reviews")
