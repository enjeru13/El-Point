...
export function isOpenNow(raw: unknown, now = new Date()): OpenState {
  ...
  if (!yest.closed && toMin(yest.close) < toMin(yest.open) && nowMin < toMin(yest.close)) {
  ...
  if (!today.closed) {
    ...
    if (nowMin < o) return { open: false, label: `Cerrado · abre hoy ${today.open}` };
  }
  ...
  for (let i = 1; i <= 7; i++) {
    ...
    if (!d.closed) {
      const when = i === 1 ? 'mañana' : DAY_LABELS_LONG[(dow + i) % 7].toLowerCase();
