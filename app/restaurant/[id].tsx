...
import { useReviews, useSubmitReview, useToggleHelpful, type Review } from '@/lib/queries/reviews';
        ...
        <View style={{
          ...
          <View style={{ padding: 20, gap: 20 }}>
            ...
            <View style={{ gap: 8 }}>
              ...
              />
              <Text style={{ color: comment.trim().length < 10 ? C.error : C.outline, fontFamily: 'PlusJakartaSans_600SemiBold', fontSize: 12, textAlign: 'right' }}>
                {comment.length < 10 ? `Mínimo 10 caracteres · ${comment.length} / 500` : `${comment.length} / 500`}
              </Text>
            </View>

            {errorMessage && (
              <Text style={{ color: C.error, fontFamily: 'PlusJakartaSans_600SemiBold', fontSize: 13 }}>
                {errorMessage}
              </Text>
            )}
      ...
      <View style={{ flex: 1, backgroundColor: C.surface, alignItems: 'center', justifyContent: 'center', gap: 16, padding: 24 }}>
        ...
        <View style={{ flexDirection: 'row', gap: 10 }}>
          <Pressable
            ...
            style={{ paddingHorizontal: 20, paddingVertical: 10, borderRadius: 99, backgroundColor: C.primary, borderWidth: 2, borderColor: C.border }}
          >
            <Text style={{ color: C.onPrimary, fontFamily: 'PlusJakartaSans_700Bold', fontSize: 14 }}>Reintentar</Text>
          </Pressable>
          <Pressable
            onPress={() => router.back()}
          ...
          >
            <Text style={{ color: C.primary, fontFamily: 'PlusJakartaSans_700Bold', fontSize: 14 }}>Volver</Text>
          </Pressable>
        </View>
      </View>
    );
  ...
  return (
    <View style={{ flex: 1, backgroundColor: C.surface }}>
      ...
      >
        ...
        >
          <Icon name={heroIcon} size={120} color={C.onSurface} style={{ opacity: 0.2 }} />
        ...
        <View style={{ padding: 16, gap: 20 }}>
          ...
          <View style={{ flexDirection: 'row', gap: 10 }}>
            ...
            <Pressable
              ...
              onPress={() => restaurant.address && Linking.openURL(`https://maps.google.com/?q=${encodeURIComponent(restaurant.address)}`)}
              style={{
          ...
          >
            ...
            {restaurant.address && (
              ...
              >
                <View style={{ width: 36, height: 36, borderRadius: 10, backgroundColor: C.primaryFixed, alignItems: 'center', justifyContent: 'center' }}>
            ...
            {restaurant.phone && (
              ...
              >
                <View style={{ width: 36, height: 36, borderRadius: 10, backgroundColor: C.secondaryContainer, alignItems: 'center', justifyContent: 'center' }}>
            ...
            {restaurant.whatsapp && (
              <Pressable
                onPress={() => Linking.openURL(`https://wa.me/${restaurant.whatsapp!.replace(/[^\d]/g, '')}`)}
                style={{
                  flexDirection: 'row', alignItems: 'center', gap: 14,
                  padding: 16,
                  borderBottomWidth: 2, borderBottomColor: C.outlineVariant,
            ...
            {restaurant.instagram && (
              <Pressable
                onPress={() => Linking.openURL(`https://instagram.com/${restaurant.instagram!.replace(/^@/, '')}`)}
                style={{
                  flexDirection: 'row', alignItems: 'center', gap: 14,
                  padding: 16,
                }}
              >
                <View style={{ width: 36, height: 36, borderRadius: 10, backgroundColor: C.tertiaryContainer, alignItems: 'center', justifyContent: 'center' }}>
          ...
          <View style={{ gap: 12 }}>
            ...
            {reviewsQ.isLoading ? (
              ...
              <View style={{ paddingVertical: 32, alignItems: 'center', gap: 8 }}>
                <Icon name="comment-text-multiple" size={36} color={C.outlineVariant} />
                <Text style={{ color: C.outline, fontFamily: 'PlusJakartaSans_400Regular', fontSize: 14, textAlign: 'center' }}>
              ...
              reviews.map(r => (
                ...
                >
                  ...
                  {/* Helpful */}
                  <Pressable
                    onPress={() => toggleHelpful.mutate({ reviewId: r.id, marked: r.viewer_marked_helpful })}
                    style={{ flexDirection: 'row', alignItems: 'center', gap: 6, alignSelf: 'flex-start' }}
                  >
                    <Icon
                      name={r.viewer_marked_helpful ? 'thumb-up' : 'thumb-up-outline'}
                      size={16}
                      color={r.viewer_marked_helpful ? C.secondary : C.outline}
                    />
                    <Text style={{ color: r.viewer_marked_helpful ? C.secondary : C.outline, fontFamily: 'PlusJakartaSans_600SemiBold', fontSize: 15 }}>
      ...
      <ReviewModal
        ...
        errorMessage={submitReview.isError ? 'No se pudo publicar. ¿Iniciaste sesión?' : null}
